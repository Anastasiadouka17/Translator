...
def raisedpos_to_binary(s):
    ...
    for e in s:
        index = int(e) - 1
        to_binary = to_binary[:index] + '1' + to_binary[index+1:]
...
def ostring_to_unicode(s):
    ...
    '''
    if(is_ostring(s)):
        s_raisedp= ostring_to_raisedpos(s)
        s_binary= raisedpos_to_binary(s_raisedp)
        s_hex= binary_to_hex(s_binary)
        s_uni= hex_to_unicode(s_hex)
        return s_uni
    else:
        return s
