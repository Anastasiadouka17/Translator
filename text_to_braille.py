...
def word_to_braille(word):
    ...
    if (is_all_caps(word)):
        cap = (CAPITAL*2)
        for c in word:
            ostring_word += char_to_braille(c)
        return cap + ostring_word
    else:
        for c in word: #c is each character in the word
            if (is_digit(c)):
                ostring_word += NUMBER + char_to_braille(c)
            elif (word[0]==c.upper()):
                ostring_word += CAPITAL + char_to_braille(c)
            else:
                ostring_word += char_to_braille(c)
        return ostring_word
...
def new_filename(fname, addition):
    ...
    '''
    return fname[:-4] + "_" + addition + fname[-4:]
